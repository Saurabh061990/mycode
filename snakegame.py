...
# we will define moves of the snake
  ...
  def setDirection(self, direction):
    if (self.direction == KEY["RIGHT"] and direction == KEY["LEFT"]
        or self.direction == KEY["LEFT"] and direction == KEY["RIGHT"]):
      pass
  ...
  def checkCrashing(self):
    ...
    while (counter < len(self.stack) - 1):
      if (checkCollision(self.stack[0], SNAKE_SIZE, self.stack[counter],
                         SNAKE_SIZE) and self.stack[counter].color != "NULL"):
        return True
      counter += 1
    return False
  ...
  def draw(self, screen):
    ...
    while (counter < len(self.stack)):
      if (self.stack[counter].color == "NULL"):
        ...
        continue
      pygame.draw.rect(
        screen, pygame.color.Color("yellow"),
...
def main():
  ...
  apples = [
    Apple(random.randint(60, SCREEN_WIDTH), random.randint(60, SCREEN_HEIGHT),
          1)
  ]
